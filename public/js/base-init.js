...
$(document).ready(function () {
	...
	$('#search-box').on('input',function() {
		var keyVal = {search : $('#search-box').val() };
		var myArr;
		$('#result').empty();
		if($('#search-box').val() == "") {
			$('#result').empty();
            ...
            $.ajax({
                ...
                success : (data) => {
                    ...
                    for(var i = 0 ; i < myArr.length ; i++) {
						$('#result').append('<li id="'+myArr[i]._id+'" class="list-group-item link-class"><img src="uploads/profile_pic/'+myArr[i].profilePic+'" height="40" width="40" class="img-thumbnail" /> '+myArr[i].firstName+' | <span class="text-muted">'+myArr[i].userName+'</span></li>');
                    }
                }
			});
        }
	...
	// 			{image: 'img/avatar30-sm.jpg', name: 'Marie Davidson', message:'4 Friends in Common', icon:'olymp-happy-face-icon'},
